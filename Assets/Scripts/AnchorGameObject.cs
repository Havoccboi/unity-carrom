...
public class AnchorGameObject : MonoBehaviour
{
	...
	IEnumerator UpdateAnchorAsync()
	{
		...
		{
			print(string.Format("CameraAnchor found CameraViewportHandler instance after waiting {0} frame(s). You might want to check that CameraViewportHandler has an earlie execution order.", cameraWaitCycles));
		}
		UpdateAnchor();
